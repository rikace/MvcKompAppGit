...
function MvcControlsToolkit_Validate(fieldName, validationType) {
    if (validationType == ValidationType_StandardClient) {
        ...
        var errorMessages = document.getElementById(fieldName)[MvcControlsToolkit_FieldContext_Tag].validate('blur');
        ...
        return $(selector).parents('form').validate().element(selector);
    }
    return true;
}
...
function MvcControlsToolkit_SortableList_AddNew(root) {
    MvcControlsToolkit_SortableList_AddNewChoice(root, 0);
}
function MvcControlsToolkit_SortableList_AddNewChoice(root, choice) {
    ...
    var result;
    var hasFooter = false;
    var lastChild = $('#' + root + SortableList_ItemsContainerPrefix).children(":last-child");
    if (lastChild.length > 0) {
        var nodeId = lastChild.attr("id");
        if (nodeId == "") lastChild = lastChild.children(":last-child");
        if (lastChild.length > 0) {
            nodeId = lastChild.attr("id");
            if (nodeId != null) {
                var end_prefix = nodeId.lastIndexOf("_");
                if (end_prefix >= 0) {
                    var ending = nodeId.substring(end_prefix + 1);
                    if (ending == 'Footer') hasFooter = true;
                }
            }
        }
    }
...
function MvcControlsToolkit_TreeView_StartDrag(item, jQueryRoot) {
    ...
    jQueryRoot.sortable('option', 'items', '#' + item.id);
...
function MvcControlsToolkit_TreeView_PrepareTemplates(root, templatesId) {
    ...
    for (var i = 0; i < templatesId.length; i++) {
        ...
        var temp = $('<div>').append($('#' + templateId).children().clone()).remove().html();
        eval(root + SortableList_TemplateHtmlPrefix + '[i] = temp;');
...
function MvcControlsToolkit_TreeView_AddNewChoice(rootName, templateChosen) {
    ...
    templateSymbol = eval(root + SortableList_TemplateSymbolPrefix);
...
function ViewList_Client(groupName, hiddenField, cssSelected, prefix) {
    ...
    allViews.each(function (i) {
        ...
        $('.' + thisId + "_checkbox").click(function () {
            if ($("."+thisId + "_checkbox").attr("checked"))
...
function ViewsOnOff_Client_Switch(groupName, on, hidden) {
    if (on) {
        $('.' + groupName + '_checkbox').attr('checked', true);
        ...
        $('.' + groupName + '_checkbox').attr('checked', false);
        $('.' + groupName).detach();
