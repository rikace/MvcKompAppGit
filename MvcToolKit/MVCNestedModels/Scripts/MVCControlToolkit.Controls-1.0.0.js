...
function MvcControlsToolkit_ToString(value, format, dataType) {
    if (dataType == MvcControlsToolkit_DataType_String || dataType < 0) return value;
    if (value == null) return '';
    if (format == '') {
...
///////////////////////////Templates/////////////////////////////
function MvcControlsToolkit_InitializeCreatedNodes(elements, unobtrusiveValidation) {
    if (elements == null) return;
    toApply = function () {
        for (var i = 0; i < elements.length; i++) {
            if (unobtrusiveValidation) {
                jQuery.validator.unobtrusive.parse(elements[i]);
            }
            GlobalEvalScriptAndDestroy(elements[i]);
            var jel = $(elements[i]);
